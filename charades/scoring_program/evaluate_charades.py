...
class LocalizationEvaluator(CharadesEvaluator):
    ...
    def build_aligned_submission_array(self):
        aligned_subm_array = np.zeros((len(self.vid_ids) * self.n_frames, self.n_classes))
        ...
        for gidx, g in enumerate(self.vid_ids):
            ...
            sort_subm_img_df = subm_img_df.sort_values('frame_n')
            aligned_subm_array[gidx * self.n_frames: (gidx + 1) * self.n_frames:] = sort_subm_img_df.values[:, 2:]
...
class ClassificationEvaluator(CharadesEvaluator):
    ...
    def build_aligned_submission_array(self):
        aligned_subm_array = np.zeros((len(self.vid_ids) * self.n_frames, self.n_classes))
        for gidx, g in enumerate(self.vid_ids):
            subm_img_df =self.submission[self.submission['frame_id'] == g]
            aligned_subm_array[gidx * self.n_frames: (gidx + 1) * self.n_frames:] = subm_img_df.values[:, 1:]
        self.submission_array = aligned_subm_array
